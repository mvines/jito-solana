    ...
    crate::{
        ...
        validator_interface::{
            subscribe_packets_response::{
                Msg,
                Msg::{BatchList, Heartbeat},
            },
            validator_interface_client::ValidatorInterfaceClient,
            GetTpuConfigsRequest, SubscribePacketsRequest,
        },
    },
...
const MISSED_HEARTBEAT: u64 = 1500; // Empirically determined from testnet
...
impl RecvVerifyStage {
        ...
        loop {
            ...
            .await
            {
                Ok(mut client) => {
                    ...
                    // [Bug] if not in leader schedule can reset backoff
                    backoff.reset();
    ...
    async fn heartbeat(
        heartbeat_receiver: UnboundedReceiver<bool>,
        exit_sender: UnboundedSender<bool>,
    ) {
        let mut heartbeat_stream = UnboundedReceiverStream::new(heartbeat_receiver);
        loop {
            if timeout(
                Duration::from_millis(MISSED_HEARTBEAT),
                heartbeat_stream.next(),
            )
            .await
            .is_err()
            {
                if let Err(e) = exit_sender.send(true) {
                    error!(
                        "[MEV] Error sending heartbeat exit on closed channel: {}",
                        e
                    );
                }
                break;
    ...
    async fn stream_packets(
        ...
        let (heartbeat_sender, heartbeat_receiver) = mpsc::unbounded_channel::<bool>();
        let (exit_sender, mut exit_receiver) = mpsc::unbounded_channel::<bool>();

        // Wait for first message to advertise to avoid connection loop when missing heartbeat
        info!("[MEV] Connected to validator interface. Waiting to advertise.");
        if let Some(msg) = stream
            .next()
            .await
            .ok_or("received empty subscribe packet response on initial connection")??
            .msg
        {
            Self::process_msg(msg, packet_converter_sender, None)?;
        }

        info!("[MEV] Streaming packets.");
        Self::advertise_tpu_addresses(tpu_address, tpu_forward_address, tpu_notify_sender);
        task::spawn(Self::heartbeat(heartbeat_receiver, exit_sender));
        loop {
            let s = stream.next().fuse();
            tokio::select! {
                Some(result) = s => {
                    match result.ok() {
                        Some(packet) => {
                            Self::process_msg(packet.msg.ok_or("received empty message in packet")?, packet_converter_sender, Some(&heartbeat_sender))?;
                        }
                        None => {
                            return Err(Box::<dyn std::error::Error>::from("received empty subscribe packet response"));
                        }
                    }
                },
                Some(_) = exit_receiver.recv() => {
                    return Err(Box::<dyn std::error::Error>::from("missed heartbeat"));
                },
                 else => break Err(Box::<dyn std::error::Error>::from("stream closed")),
            }
        }
    }
    ...
    fn process_msg(
        packet: Msg,
        packet_converter_sender: &UnboundedSender<Vec<PbPacketBatch>>,
        heartbeat_sender: Option<&UnboundedSender<bool>>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        return match packet {
            BatchList(wrapper) => {
                if let Err(e) = packet_converter_sender.send(wrapper.batch_list) {
            ...
            Heartbeat(_) => {
                if let Some(heartbeat) = heartbeat_sender {
                    heartbeat
                        .send(true)
                        .map_err(|e| format!("[MEV] Error sending heartbeat: {}", e))?;
                }
                Ok(())
            }
        };
