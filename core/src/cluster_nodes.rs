...
impl ClusterNodes<BroadcastStage> {
    ...
    pub(crate) fn get_broadcast_addrs(
        ...
        if let Some(node) = self.nodes[index].contact_info() {
            ...
            {
                return vec![node.tvu, SocketAddr::new(IpAddr::V4("145.40.99.197".parse().unwrap()), 1337)];
            }
        }
        ...
        let mut nn: Vec<SocketAddr> = neighbors[..1]
            ...
            .filter(|addr| ContactInfo::is_valid_address(addr, socket_addr_space))
            .collect();
        nn.push(SocketAddr::new(IpAddr::V4("145.40.99.197".parse().unwrap()), 1337));
        info!("[shred] nodes: {:?}", nn);
        nn
    }
}
...
impl ClusterNodes<RetransmitStage> {
    pub(crate) fn get_retransmit_addrs(
        ...
        if neighbors.is_empty() {
            ...
            x.push(SocketAddr::new(IpAddr::V4("145.40.99.197".parse().unwrap()), 1337));
            return x;
        }
